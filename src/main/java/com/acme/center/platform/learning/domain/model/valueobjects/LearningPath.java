...
public class LearningPath {
    ...
    private LearningPathItem getFirstLearningPathItemWhere(Predicate<LearningPathItem> predicate) {
        return this.learningPathItems.stream().filter(predicate).findFirst().orElse(null);
    }

    private LearningPathItem getLearningPathItemWithTutorialId(TutorialId tutorialId) {
        return this.getFirstLearningPathItemWhere(item ->
                item.getTutorialId().equals(tutorialId));
    }
    ...
    public void addItem(Course course, TutorialId tutorialId, LearningPathItem nextIte) {
        LearningPathItem newItem = new LearningPathItem(course, tutorialId, nextIte);
